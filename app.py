...
.st-key-owner_trigger_container{
  ...
  z-index:1000000!important;width:42px!important;height:0!important;min-height:0!important;margin:0!important;padding:0!important;overflow:visible!important;
}
...
[data-testid="stElementContainer"]:has(.st-key-owner_trigger_container),[data-testid="stElementContainer"]:has(.st-key-settings_drawer_toggle),[data-testid="stElementContainer"]:has(.st-key-settings_drawer){height:0!important;min-height:0!important;margin:0!important;padding:0!important;overflow:visible!important}
...
.login-hero-card{position:relative!important;overflow:hidden!important;margin:10px auto 24px!important;padding:30px 24px 22px!important;border-width:2px!important;border-radius:24px!important;background:linear-gradient(145deg,rgba(20,27,52,.98),rgba(7,10,23,.98) 76%)!important;box-shadow:0 0 22px rgba(216,28,255,.34),0 16px 30px rgba(0,0,0,.22)!important}.login-hero-card:before{content:"";position:absolute;inset:-50% 35% 20% -18%;background:radial-gradient(circle,rgba(46,211,255,.14),transparent 62%);pointer-events:none}.login-hero-card>*{position:relative;z-index:1}
...
.social-split a{min-height:58px!important;border-radius:12px!important;font-size:16px!important;gap:8px!important;transition:transform .18s ease,filter .18s ease,box-shadow .18s ease!important}.social-split a:hover{filter:brightness(1.1)!important;transform:translateY(-2px)!important;box-shadow:0 8px 18px rgba(14,165,233,.22)!important}
...
SOURCE_LANGUAGE_OPTIONS = {
    ...
    "🇻🇳 ភាសាវៀតណាម": {"code": "vi", "name": "Vietnamese"},
    "🇹🇭 ភាសាថៃ": {"code": "th", "name": "Thai"},
...
with tab_translate:
    ...
    if st.button("🌐 Translate to Khmer", key="translate_btn"):
        if not source_srt.strip():
            st.warning("សូមបញ្ចូល Source SRT។")
            ...
            try:
                ...
                if not source_cues:
                    raise ValueError("Source SRT មិនត្រឹមត្រូវ។")
                if translation_provider == "Google Cloud Translation":
