...
# encrypted customer API keys unreadable.
...
cipher_secrets = [primary_cookie_secret]
if primary_cookie_secret != LEGACY_COOKIE_SECRET:
    cipher_secrets.append(LEGACY_COOKIE_SECRET)
api_ciphers = [
...
def character_voice_filters(tag):
    ...
    mapping = {
        ...
        'M_THINK': [
            ...
            'lowpass=f=6800:p=2', 'volume=0.84'
        ],
        'F_THINK': [
...
def create_mp3(srt_text, progress_callback=None):
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        for index, cue in enumerate(cues):
            ...
            parts.extend([
                # Keep dialogue warm and intelligible while reducing sharp, airy hiss.
                'highpass=f=85:p=2',
                'lowpass=f=7000:p=2',
...
def create_access_code_batch(prefix, count, duration_days, plan_label=""):
    """Create up to 1,000 unique customer access codes in one short database transaction."""
    base = normalize_access_code(prefix).strip("-")
    if not base:
        raise ValueError("សូមបញ្ចូល Prefix សម្រាប់ Access Code។")
    amount = int(count)
    if amount < 1 or amount > 1000:
        raise ValueError("ចំនួន Access Code ត្រូវមានចន្លោះពី 1 ដល់ 1,000។")
    days = int(duration_days)
    allowed_plans = {7: "7 ថ្ងៃ", 30: "1 ខែ", 90: "3 ខែ", 180: "6 ខែ", 365: "1 ឆ្នាំ"}
    ...
    with license_connection() as connection:
        existing_codes = {
            str(row["access_code_display"])
            for row in connection.execute("SELECT access_code_display FROM licenses WHERE access_code_display LIKE ?", (f"{base}-%",))
        }
        serial = 1
        while len(created) < amount:
            code = f"{base}-{serial:04d}"
            serial += 1
        ...
        records = [
            (
                f"{base} {index:04d}", _hash_code(code), code, _iso(now), _iso(expires),
                _iso(card_until), plan,
            )
            for index, code in enumerate(created, start=1)
        ]
    ...
    _audit("license_batch_created", get_admin_username(), f"prefix={base}|count={amount}|{plan}")
...
def admin_dashboard():
    ...
    with st.expander("⚡ បង្កើត Access Code ជាក្រុម", expanded=False):
        st.caption("បង្កើតកូដដាច់ដោយឡែករហូតដល់ 1,000 កូដក្នុងម្តង។ កូដនីមួយៗគឺសម្រាប់អ្នកប្រើម្នាក់។")
        with st.form("bulk_license_form", clear_on_submit=True):
            batch_prefix = st.text_input("Prefix", value="KHBR", help="ឧ. KHBR នឹងបង្កើត KHBR-0001, KHBR-0002 …")
            batch_count = st.selectbox("ចំនួនកូដ", [10, 25, 50, 100, 250, 500, 1000], index=3)
            batch_duration = st.selectbox("រយៈពេលកូដ", ["7 ថ្ងៃ", "1 ខែ", "3 ខែ", "6 ខែ", "1 ឆ្នាំ"], key="bulk_duration")
            batch_submit = st.form_submit_button("បង្កើត Access Code ជាក្រុម", use_container_width=True)
        if batch_submit:
            try:
                batch_days = {"7 ថ្ងៃ": 7, "1 ខែ": 30, "3 ខែ": 90, "6 ខែ": 180, "1 ឆ្នាំ": 365}[batch_duration]
                batch_codes, batch_expiry = create_access_code_batch(batch_prefix, batch_count, batch_days, batch_duration)
                st.session_state.bulk_access_codes = batch_codes
                st.session_state.bulk_access_expiry = _iso(batch_expiry)
                st.success(f"បានបង្កើត Access Code ដាច់ដោយឡែក {len(batch_codes):,} កូដរួចរាល់។")
            except Exception as exc:
                st.error(f"មិនអាចបង្កើតកូដជាក្រុមបាន៖ {exc}")
        batch_codes = st.session_state.get("bulk_access_codes", [])
        if batch_codes:
            export_text = "Access Code\n" + "\n".join(batch_codes) + "\n"
            st.download_button(
                "⬇️ ទាញយក Access Code (.csv)", export_text.encode("utf-8"),
                file_name="ai_khemra_access_codes.csv", mime="text/csv",
                key="download_bulk_access_codes", use_container_width=True,
            )
            st.caption(f"Expiry: {_parse_iso(st.session_state.get('bulk_access_expiry')).astimezone().strftime('%Y-%m-%d %H:%M')}")
            ...
            if st.button("🚀 ចាប់ផ្ដើមលំហូរដែលបានជ្រើស", key="generate_srt", use_container_width=True):
                ...
                try:
                    ...
                    if workflow_mode == "🎙️ Khmer SRT + MP3 តែម្តង" and generated_srt != source_srt:
                        ...
                        try:
                            generated_audio = create_mp3(generated_srt, progress_callback=auto_audio_progress)
...
with tab_translate:
    ...
    if st.button("🌐 Translate SRT to Khmer", key="translate_btn"):
...
st.caption("AI-KHEMRA-BRO v6.4 • Multilingual → Khmer • 4 Voice Tags • Mobile-first")
