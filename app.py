...
internal thought are allowed. Do not use any other tag.

RULE 6 — KHMER-ONLY OUTPUT: The text field must contain Khmer script only. Never leave
Chinese, Korean, Thai, Vietnamese, English, romanization, translator notes, or brackets
inside the dialogue. Return one JSON object for every supplied ID in the same order.
""".strip()
...
def _clean_api_keys(api_keys_text):
    ...
    keys = []
    seen = set()
    for raw_key in str(api_keys_text or "").splitlines():
        key = raw_key.strip()
...
def extract_audio(video_path, audio_path, fast_mode=True):
    """Extract a Whisper-ready mono track; fast mode avoids costly denoise/FLAC encoding."""
    if fast_mode:
        command = [
        ...
        command = [
            "ffmpeg", "-y", "-i", str(video_path), "-vn", "-ac", "1", "-ar", "16000",
            "-af", audio_filter, "-c:a", "flac", "-compression_level", "5", str(audio_path),
        ]
