...
'M_THINK':{'voice':PISITH,'rate':'-13%','pitch':'-7Hz','volume':'-5%'},
'F_THINK':{'voice':SREYMOM,'rate':'-13%','pitch':'-6Hz','volume':'-5%'},
...
unheard internal monologue or thought voice, never just because an actor speaks quietly.
...
def save_private_api_keys(api_keys_text):
    ...
    try:
        ...
        pass
    # Keep the browser copy as a fallback, but do not report permanent saving
    # as successful unless the encrypted account database update succeeded.
...
def character_voice_filters(tag):
    ...
    mapping = {
        ...
        'M_THINK': [
            'equalizer=f=180:t=q:w=1.0:g=1.0', 'equalizer=f=3200:t=q:w=1.0:g=-1.5',
            'lowpass=f=5600:p=2', 'volume=0.78', 'aecho=0.60:0.70:55:0.16'
        ],
        'F_THINK': [
            'equalizer=f=220:t=q:w=1.0:g=0.6', 'equalizer=f=3400:t=q:w=1.0:g=-1.5',
            'lowpass=f=5800:p=2', 'volume=0.78', 'aecho=0.60:0.70:55:0.16'
        ],
                ...
                try:
                    ...
                    if workflow_mode == "🎙️ Khmer SRT + MP3 តែម្តង" and generated_srt != source_srt:
                        def auto_audio_progress(percent, message):
                            ...
                            progress_text.markdown(f"### 🎙️ {message}", unsafe_allow_html=True)
                        try:
