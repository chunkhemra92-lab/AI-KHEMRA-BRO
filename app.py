...
real speaker pauses. Do not make the Khmer voice rush, drag, flatten its tone, or jump
between artificial highs and lows.
...
def create_mp3(srt_text, progress_callback=None):
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        # Master once after mixing.  The leveler works across long, overlapping
        # 800 ms frames and permits only a small gain change, so adjacent cues
        # do not jump in loudness while syllable-level intonation stays intact.
        # It is deliberately not a per-word normalizer.
        filters.append(
            ''.join(labels)
...
def create_single_voice_mp3(text, tag):
    """Create a polished standalone MP3 so Text → Speech uses the same quality chain."""
    canonical_tag = normalize_voice_tag(tag)
    with tempfile.TemporaryDirectory() as folder:
        root = Path(folder)
        ...
        filter_chain = (
            ...
            + ',' + FINAL_LEVELER_FILTER
            + ',alimiter=limit=0.92:attack=25:release=320'
            + ',loudnorm=I=-17:TP=-2.0:LRA=7[out]'
        )
        result = subprocess.run(
            [
                'ffmpeg', '-hide_banner', '-loglevel', 'error', '-nostdin', '-y',
                '-i', str(raw), '-filter_complex', filter_chain, '-map', '[out]',
...
st.caption("AI-KHEMRA-BRO v6.5.0 • Natural Audio Leveling • Multilingual → Khmer • 4 Voice Tags • Mobile-first")
