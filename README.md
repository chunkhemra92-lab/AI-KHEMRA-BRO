...
Replace the deployed `app.py`, `requirements.txt`, and `packages.txt` with the matching files in this release. Place all three files in the **same repository folder** as `app.py`, then reboot or redeploy the Streamlit service. The included `requirements.txt` explicitly installs `edge-tts` so the speech features can start on Streamlit Cloud.
