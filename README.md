...
The app reads Railway Variables privately at runtime. `pydub` is included in `requirements.txt` so Railway Railpack detects the required FFmpeg system package. After deploy succeeds, choose **Settings → Networking → Public Networking → Generate Domain** to remove the “Unexposed service” status.
