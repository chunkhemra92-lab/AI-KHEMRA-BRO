...
Open **☰ Settings** to select the Gemini model, target language, translation style, source language, workflow, and processing mode. In v6.7.11, the Settings button has a clear label, the mobile sheet is wider but remains compact, and dropdowns, expandable rows, and the API-key entry area have larger touch targets. The target language, Gemini model, and translation style are private to the signed-in customer.
